export const Element = (tag) => (properties = {}) => (content = '') => (`<${tag}${Object.values(properties).length > 0 ? ' ' : ''}${Object.entries(properties).map(e => `${e[0]}="${e[1]}"`).join(' ')}${content === '' ? '/' : ''}>`
...
export const format = (xml, indent = 2) => {
